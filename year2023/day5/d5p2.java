...
public class d5p2 {
	public static void main(String[] args) throws FileNotFoundException {
        ...
        for(int i = 0; i < seeds.size(); i += 2) {
        	System.out.println("We're at " + seeds.get(i));
        	for(long j = seeds.get(i + 1); j >= 0; j--) {
        		long currSeed = seeds.get(i) + j;
        		
	        	for(ArrayList<Long> sts : seedToSoilMap) {
	        		if(currSeed >= sts.get(1) && currSeed <= sts.get(1) + sts.get(2)) {
	        			currSeed = currSeed + (sts.get(0) - sts.get(1)) ;
	        			break;
	        		}
	        	}
	        	
	        	for(ArrayList<Long> stf : soilToFertMap) {
	        		if(currSeed >= stf.get(1) && currSeed <= stf.get(1) + stf.get(2)) {
	        			currSeed = currSeed + (stf.get(0) - stf.get(1));
	        			break;
	        		}
	        	}
	        	
	        	for(ArrayList<Long> ftw : fertToWaterMap) {
	        		if(currSeed >= ftw.get(1) && currSeed <= ftw.get(1) + ftw.get(2)) {
	        			currSeed = currSeed + (ftw.get(0) - ftw.get(1));
	        			break;
	        		}
	        	}
	        	
	        	for(ArrayList<Long> wtl : waterToLightMap) {
	        		if(currSeed >= wtl.get(1) && currSeed <= wtl.get(1) + wtl.get(2)) {
	        			currSeed = currSeed + (wtl.get(0) - wtl.get(1));
	        			break;
	        		}
	        	}
	        	
	        	for(ArrayList<Long> ltt : lightToTempMap) {
	        		if(currSeed >= ltt.get(1) && currSeed <= ltt.get(1) + ltt.get(2)) {
	        			currSeed = currSeed + (ltt.get(0) - ltt.get(1));
	        			break;
	        		}
	        	}
	        	
	        	for(ArrayList<Long> tth : tempToHumMap) {
	        		if(currSeed >= tth.get(1) && currSeed <= tth.get(1) + tth.get(2)) {
	        			currSeed = currSeed + (tth.get(0) - tth.get(1));
	        			break;
	        		}
	        	}
	        	
	        	for(ArrayList<Long> htl : humToLocMap) {
	        		if(currSeed >= htl.get(1) && currSeed <= htl.get(1) + htl.get(2)) {
	        			currSeed = currSeed + (htl.get(0) - htl.get(1));
	        			break;
	        		}
	        	}
	        	if(currSeed < ret) {
